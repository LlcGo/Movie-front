  ...
  <div class="home-box">
    ...
    <el-card>
      <el-table :data="tableData"
        ...
        <el-table-column prop="nationName" label="类型名" width="180"></el-table-column>
    ...
    >
      ...
      >
        <el-form-item label="类型名">
          <el-input v-model="formLabelAlign.nationName"/>
        </el-form-item>
      </el-form>
      <template #footer>
...
<!--    新增-->
    ...
    >
      ...
      >
        <el-form-item label="类型名">
          <el-input v-model="formAddLabelAlign.nationName"/>
        </el-form-item>
      </el-form>
      <template #footer>
      <span class="dialog-footer">
...
const formAddLabelAlign = ref({nationName:''});
onMounted(() => {
  // alert(1)
  getListData();
});
...
const toAdd = async () => {
  const res =await MovieNationControllerService.addMoveNationUsingPost(formAddLabelAlign.value.nationName);
  if(res.data){
...
const getListData = async () => {
  ...
  // return
  const res = await MovieNationControllerService.listMoveNationByPageUsingPost(searchForm.value)
...
const handleAddClose = () => {
   formAddLabelAlign.value.nationName = '';
...
const onShowClick = async (row) => {
  ...
  const res =  await MovieNationControllerService.updateMoveNationUsingPost(row.id);
...
const toEdit = async () => {
  ...
  if (formLabelAlign.value.nationName == '') {
    message.warn('请输入类型名');
    return;
  }
  console.log(formLabelAlign.value);
  // return
  const res = await MovieNationControllerService.updateMoveNationUsingPost1(formLabelAlign.value)
  if (res.data) {
...
const remove = row => {
  ...
  ElMessageBox.confirm(
      "确定将" + row.nationName + mes,
      {type: 'warning'}
  ).then(async () => {
    const res = await MovieNationControllerService.deleteMoveNationUsingPost(row)
...
const getMovieTypes = async () => {
  const res = await MovieNationControllerService.getMovieNationUsingGet();
...
const searchAll = async () => {
  ...
  // return;
  // return;
  const res = await MovieNationControllerService.listMovieTypeByPageUsingPost(data)
  console.log('获得的数据', res)
  if (res.data !== null) {
    total.value = Number(res.data.total);
    tableData.value = res.data.records;
